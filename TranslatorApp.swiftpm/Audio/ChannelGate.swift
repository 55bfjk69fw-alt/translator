...
final class ChannelGate {
    ...
    func evaluate(channels: [UnsafePointer<Float>], frames: Int, sampleRate: Double, channelEnabled: [Bool]? = nil) -> [Decision] {
        ...
        for i in 0..<count {
            ...
            if isEnabled(i), useVAD, rms[i] < minimumVoiceThreshold * 0.5 {
                ...
                // forward pass continuously. The 0.5 margin keeps the VAD
                // fed (hysteresis and LSTM state intact) for a speaker
                // hovering AT the threshold; true silence sits orders of
                // magnitude below it. Clearing the hold mirrors the
