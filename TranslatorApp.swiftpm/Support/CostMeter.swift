...
final class CostMeter {
    ...
    /// identity-guarded upstream. Negative values are corrections — audio
    /// counted at hand-off that died with its connection before reaching
    /// the server is never billed there, so the client retracts it.
    /// Clamped at zero.
    func addDollars(_ amount: Double) {
        guard amount != 0 else { return }
        lock.lock(); defer { lock.unlock() }
        dollars = max(0, dollars + amount)
    }
