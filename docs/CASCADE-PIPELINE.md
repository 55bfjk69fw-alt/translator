...
protocol LaneEngine: AnyObject {
    ...
    /// with it (realtime: silence tail then append pause; cascade:
    /// drop) — and the engine owns its
...
- **Input**: the lane engine converts gate-passed buffers to
  ...
  are *not* sent (like realtime's post-tail append pause, but with no
  silence tail at all), so slots only advance during speech and idle
  lanes cost nothing.
